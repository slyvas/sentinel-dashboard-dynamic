...
public class DegradeController {
    ...
    @Autowired
    @Qualifier("degradeRuleZKProvider")
    private DynamicRuleProvider<List<DegradeRuleEntity>> ruleProvider;
    
    @Autowired
    @Qualifier("degradeRuleZKPublisher")
    ...
    private boolean publishRules(String app, String ip, Integer port) {
        ...
        try {
			...
			logger.info("add rules success", JSON.toJSONString(rules.stream().map(DegradeRuleEntity::toRule).collect(Collectors.toList())));
		} catch (Exception e) {
			logger.info("add rules success", JSON.toJSONString(rules.stream().map(DegradeRuleEntity::toRule).collect(Collectors.toList())), e);
			return false;
		}
        return true;
